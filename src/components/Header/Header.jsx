...
const Header = () => {
  ...
  useEffect(() => {
    ...
    if (username) {
      ...
      setUserData(user); // Устанавливаем данные пользователя из localStorage
    }
  }, []);
  ...
  const handleLogout = () => {
    localStorage.removeItem('loggedInUsername'); // Удаляем информацию о пользователе
    setUserData(null); // Сбрасываем состояние данных пользователя
              ...
              {!userData ? (
                <>
                  ...
                  <Link onClick={CloseMenu} to={'/register'}>
                    <button className={s.reg}>Регистрация</button>
                  </Link>
                </>
              ) : (
